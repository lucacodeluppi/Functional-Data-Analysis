...
if (length(group_A_fds) > 0 & length(group_B_fds) > 0) {
  ...
  # Compute mean curves (no normalization) 
  ...
  # Plot mean curves for the two groups 
  plot(
    mean_A,
    col  = "blue",
    lwd  = 2,
...
#point_cloud <- sliding_window(ts_scaled, dim = 3, delay = 1)
# detrending
...
# Plot Persistence Barcodes (H0 e H1)
plot(diag[["diagram"]],
     barcode = TRUE,
