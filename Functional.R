...
for (yr in years) {
  ...
  # B-spline basis
  basis_spline <- create.bspline.basis(rangeval = rangeval,
                                       nbasis   = nbasis,
                                       norder   = norder)
...
# Step 4.3: Performing Functional PCA (FPCA)
...
nbasis <- 5   # you have 5 age points → 5 basis functions is reasonable
...
# Step 4.4 - Functional Regression (FRegress)
# Functional Regression 
# For each Year–Week, build two functions of age:
# – 𝑌(age) deaths by age (levels)
# – X (age) = proportion by age
# Then do functional regression: deaths_profile(age) ~ proportion_profile(age)
# Create a list to store regression results
fregress_results_list <- list()

# B-spline parameters over AGE 
norder <- 4          # cubic B-splines
nbasis <- 5          # 5 basis functions (one per age group)

# Age grid: midpoints of age classes
age_grid <- c(7, 40, 70, 80, 90)   

# Common basis over age (same for all weeks/years)
rangeval <- range(age_grid)
basis_spline <- create.bspline.basis(rangeval = rangeval,
                                     nbasis   = nbasis,
                                     norder   = norder)

# Work with both sexes (b)
aus_both <- aus %>% filter(Sex == "b")

# Loop over years
for (yr in sort(unique(aus_both$Year))) {
  
  df_year <- aus_both %>%
    filter(Year == yr)
  
  weeks <- sort(unique(df_year$Week))
  
  # Lists of weekly functional data for this year
  weekly_deaths_fd_list <- list()
  weekly_prop_fd_list   <- list()
  ...
  for (wk in weeks) {
    ...
    # Should be exactly one row per (Year, Week, Sex)
    if (nrow(df_week) == 1) {
      
      # Response: deaths by age (level)
      y_values <- c(
        df_week$age_0_14,
        df_week$age_15_64,
        df_week$age_65_74,
      ...
      # Predictor: proportions by age
      x_values <- c(
        df_week$prop_0_14,
        df_week$prop_15_64,
        df_week$prop_65_74,
        df_week$prop_75_84,
        df_week$prop_85_plus
      )
      
      # Smooth deaths profile over age
      deaths_fd <- smooth.basis(
        age_grid, y_values, fdPar(basis_spline, Lfdobj = 2, lambda = 0.1)
      )$fd
      
      # Smooth proportions profile over age
      prop_fd <- smooth.basis(
        age_grid, x_values, fdPar(basis_spline, Lfdobj = 2, lambda = 0.1)
      )$fd
      
      key <- paste0(yr, "_Week", wk)
      weekly_deaths_fd_list[[key]] <- deaths_fd
      weekly_prop_fd_list[[key]]   <- prop_fd
    }
  }
  # Use only weeks for which we have both curves
  common_weeks <- intersect(names(weekly_deaths_fd_list),
                          names(weekly_prop_fd_list))
  
  if (length(common_weeks) < 3) next  # skip years with too few weeks
  
  # Combine into fd objects with one curve per week (sample = weeks)
  deaths_fd_combined <- fd(
    coef = do.call(cbind, lapply(common_weeks, function(wk_name) {
      weekly_deaths_fd_list[[wk_name]]$coefs
    })),
    basisobj = basis_spline
  )
  
  prop_fd_combined <- fd(
    coef = do.call(cbind, lapply(common_weeks, function(wk_name) {
      weekly_prop_fd_list[[wk_name]]$coefs
    })),
    basisobj = basis_spline
  )
  
  n_weeks <- length(common_weeks)
  
  # Functional regression setup:
  # Y(age, week) = beta0(age) + beta1(age) * prop(age, week) + error
  xfdlist <- list(
    const = rep(1, n_weeks),         # scalar intercept
    prop  = prop_fd_combined         # functional regressor over age
  )
  
  # Beta for intercept: constant basis over age
  betabasis_const <- create.constant.basis(rangeval)
  betafd_const    <- fd(0, betabasis_const)
  betafdPar_const <- fdPar(betafd_const)
  
  # Beta for prop: same B-spline basis over age
  betabasis_prop  <- basis_spline
  nbasis_prop     <- betabasis_prop$nbasis
  
  # beta(age) iniziale = 0 per tutte le basi
  betafdProp      <- fd(rep(0, nbasis_prop), betabasis_prop)
  betafdPar_prop  <- fdPar(betafdProp)
  betalist <- list(
    const = betafdPar_const,
    prop  = betafdPar_prop
  )
  
  # Functional regression: deaths_profile(age) ~ prop_profile(age)
  fregress_results <- fRegress(deaths_fd_combined, xfdlist, betalist)
  fregress_results_list[[as.character(yr)]] <- fregress_results
  
  # Plot estimated beta(age) for proportions
  plot(
    fregress_results$betaestlist$prop,
    xlab = "Age",
    ylab = "Beta(age) for proportion profile",
    main = paste("Functional Regression Beta(age) - Year", yr)
  )
}

# Step 4.5 – Functional ANOVA (FANOVA) using the grouping.
# ---- Defining group yearly based---- [PRE-POST COVID]
# ---- 1. Define groups based on year, from names like "2015_Week1" ----
...
# Phase 5 – TDA on weekly mortality
...
# 2) Sliding window embedding to build point cloud
...
# Apply sliding window to mortality series
point_cloud <- sliding_window(ts_data, dim = 3, delay = 1)
...
# Step 5.3.1 – Plot Persistence Barcodes (H0 e H1)
plot(diag[["diagram"]],
     barcode = TRUE,
